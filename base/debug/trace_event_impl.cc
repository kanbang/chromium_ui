...
class TraceLog::ThreadLocalEventBuffer
 ...
 private:
  ...
  void CheckThisIsCurrentBuffer() const {
    //kk DCHECK(trace_log_->thread_local_event_buffer_.Get() == this);
  }
...
void TraceLog::Flush(const TraceLog::OutputCallback& cb) {
  ...
  int generation = this->generation();
  {
    ...
    if (thread_message_loops_.size()) {
      for (unordered_set<MessageLoop*>::const_iterator it =
...
void TraceLog::AddTraceEventWithThreadIdAndTimestamp(
  ...
  if (thread_id == static_cast<int>(PlatformThread::CurrentId())) {
    ...
    if (new_name != g_current_thread_name.Get().Get() &&
      ...
      AutoLock lock(lock_);
      unordered_map<int, std::string>::iterator existing_name =
...
void TraceLog::AddMetadataEventsWhileLocked() {
  ...
  if (process_labels_.size() > 0) {
    ...
    for(base::unordered_map<int, std::string>::iterator it = process_labels_.begin();
  ...
  // Thread sort indices.
  for(unordered_map<int, int>::iterator it = thread_sort_indices_.begin();
  ...
  // Thread names.
  for(unordered_map<int, std::string>::iterator it = thread_names_.begin();
...
void TraceLog::RemoveProcessLabel(int label_id) {
  AutoLock lock(lock_);
  base::unordered_map<int, std::string>::iterator it = process_labels_.find(
      label_id);
