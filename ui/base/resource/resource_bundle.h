...
class UI_EXPORT ResourceBundle {
 public:
  ...
  // return the global css resource 
  static LU_CSSDocument& GetShareCssInstance();

  // Initialize the ResourceBundle using given data pack path for testing.
  static void InitCssSharedInstanceWithPakPath(const string16& path, const string16& file);

  // Delete the ResourceBundle for this process if it exists.
  static void CleanupCssSharedInstance();

  // Returns true after the global resource loader instance has been created.
  static bool HasCssSharedInstance();
