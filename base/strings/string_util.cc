...
std::vector<string16> split_text(
  const string16 &text,
  const string16 &split_string,
  bool skip_empty)
{
  std::vector<string16> result;
  string16::size_type end_pos = 0, begin_pos = 0;
  while (true)
  {
    end_pos = text.find(split_string, begin_pos);
    if (end_pos == string16::npos)
    {
...
string16 trim(const string16 &text)
{
  string16::size_type first_char = text.find_first_not_of(L" \r\n\t");
  string16::size_type last_char = text.find_last_not_of(L" \r\n\t");
  if (first_char == string16::npos)
    return string16();
  if (last_char == string16::npos)
    return string16();
  return text.substr(first_char, last_char - first_char + 1);
}

int compare(const std::string &a, const std::string &b, bool case_insensitive)
{
  if (case_insensitive)
...
int compare(const string16 &a, const string16 &b, bool case_insensitive)
{
  if (case_insensitive)
