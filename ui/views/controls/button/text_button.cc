...
void TextButtonBase::SetAttribute(const string16& name, const string16& v) {
  if(name == L"text")
    SetText(v);
    ...
    SetMultiLine(v == L"true");
  }
  else if(name == L"color") {
    SetEnabledColor(SkColor(base::GetColor(v)));
  }
  else if(name == L"color-hot")
    SetHoverColor(SkColor(base::GetColor(v)));
  else if(name == L"color-highlight")
    SetHighlightColor(SkColor(base::GetColor(v)));
  else if(name == L"color-disable")
    SetDisabledColor(SkColor(base::GetColor(v)));
...
void TextButton::SetAttribute(const string16& name, const string16& v) {
  if(name == L"border") {
    if(v == L"native") {
  ...
  else if(name == L"icon-place") {
    if(v == L"left")
      set_icon_placement(ICON_ON_LEFT);
    else if(v == L"center")
  ...
  } else if(name == L"icon-hot") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetHoverIcon(*icon_image);
    }
  }
  else if(name == L"icon-push") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetPushedIcon(*icon_image);
    }
  }
  else
    TextButtonBase::SetAttribute(name, v);
}
