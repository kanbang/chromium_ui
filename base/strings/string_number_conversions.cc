...
std::string GetRGBAColor(const std::string& input) {
  if(input.length()==6) {
    std::string temp_color= "FF";
    temp_color += input;
...
std::string GetRGBAColor(const string16& input) {
  std::string color = WideToUTF8(input);
  return GetRGBAColor(color);
}

uint64 GetColor(const string16& v) {
  std::string color = base::GetRGBAColor(v);
  uint64 c = 0;
  base::HexStringToUInt64(color, &c);
  return c;
}
}  // namespace base
