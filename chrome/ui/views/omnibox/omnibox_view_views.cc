...
#include "chrome/ui/views/omnibox/omnibox_popup_contents_view.h"
...
ui::TextInputType DetermineTextInputType() {
  ...
  const std::string locale = "zh";//g_browser_process->GetApplicationLocale();
...
bool OmniboxViewViews::OnMousePressed(const ui::MouseEvent& event) {
  select_all_on_mouse_release_ =
      ...
      (!HasFocus() || (model()->focus_state() == 0/*OMNIBOX_FOCUS_INVISIBLE*/));
...
void OmniboxViewViews::OnTabChanged(const content::WebContents* web_contents) {
  ...
  SetText(UTF8ToWide(web_contents->GetURL().spec()));
