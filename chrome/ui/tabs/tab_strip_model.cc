...
#include "chrome/ui/tabs/tab_strip_model_order_controller.h"
...
bool ShouldForgetOpenersForTransition(int transition) {
  return false;
...
void TabStripModel::InsertWebContentsAt(int index,
  ...
  bool pin = false;///*extensions_tab_helper->is_app() || */add_types & ADD_PINNED;
...
WebContents* TabStripModel::DiscardWebContentsAt(int index) {
  ...
  WebContents* null_contents = NULL;
      //WebContents::Create(WebContents::CreateParams(NULL));
  WebContents* old_contents = GetWebContentsAtImpl(index);
...
bool TabStripModel::IsAppTab(int index) const {
  ...
  return false;//contents && extensions::TabHelper::FromWebContents(contents)->is_app();
}
...
void TabStripModel::AddWebContents(WebContents* contents,
  ...
  if (transition == 0/*content::PAGE_TRANSITION_LINK*/ &&
  ...
  if (transition == 0/*content::PAGE_TRANSITION_TYPED*/ && index == count()) {
  ...
  if (inherit_group && transition == 0/*content::PAGE_TRANSITION_TYPED*/)
...
bool TabStripModel::IsContextMenuCommandEnabled(
  ...
  switch (command_id) {
    ...
    case CommandBookmarkAllTabs:
      return false;/*::bookmarks_enabled &&
...
bool TabStripModel::ContextMenuCommandToBrowserCommand(int cmd_id,
  ...
  switch (cmd_id) {
    case CommandNewTab:
      *browser_cmd = 1;//IDC_NEW_TAB;
      break;
    case CommandReload:
      *browser_cmd = 2;//IDC_RELOAD;
      break;
    case CommandDuplicate:
      *browser_cmd = 3;//IDC_DUPLICATE_TAB;
      break;
    case CommandCloseTab:
      *browser_cmd = 4;//IDC_CLOSE_TAB;
      break;
    case CommandRestoreTab:
      *browser_cmd = 5;//IDC_RESTORE_TAB;
      break;
    case CommandBookmarkAllTabs:
      *browser_cmd = 6;//IDC_BOOKMARK_ALL_TABS;
      break;
    default:
      *browser_cmd = 0;
      return false;
