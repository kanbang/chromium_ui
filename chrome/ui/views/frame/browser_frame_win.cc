...
#include "chrome/ui/views/frame/browser_frame_common_win.h"
#include "chrome/ui/views/frame/browser_view.h"
#include "chrome/ui/views/frame/system_menu_insertion_delegate_win.h"
...
BrowserFrameWin::BrowserFrameWin(BrowserFrame* browser_frame,
  ...
  scoped_ptr<ui::ThemeProvider> theme_provider(new themeresoure::BrowserThemeProvider());
  browser_frame_->SetThemeProvider(theme_provider.Pass());
}
...
//     profile_to_switch_to = current_profile->GetOffTheRecordProfile();
  ...
  Browser* browser_to_switch_to = chrome::FindTabbedBrowser(false, chrome::HOST_DESKTOP_TYPE_NATIVE);
