...
int NativeTextfieldViews::GetTextDirection() const {
...
bool NativeTextfieldViews::IsCommandIdEnabled(int command_id) const {
  ...
  switch (command_id) {
    ...
    case /*IDS_APP_PASTE*/3:
      ui::Clipboard::GetForCurrentThread()->ReadText(
          ui::CLIPBOARD_TYPE_COPY_PASTE, &result);
      return editable && !result.empty();
    case /*IDS_APP_DELETE*/4:
      return editable && model_->HasSelection();
    case /*IDS_APP_SELECT_ALL*/5:
...
string16 NativeTextfieldViews::GetTextForDisplay(const string16& text) {
  return textfield_->style() & Textfield::STYLE_LOWERCASE ?
      /*base::i18n::ToLower(text)*/text : text;
}
    ...
    switch (key_code) {
      ...
      case ui::VKEY_HOME:
        if ((key_code == ui::VKEY_HOME) ==
            (GetRenderText()->GetTextDirection() == 2))
