...
#include "chrome/render/web_contents_view_win.h"

#include "base/bind.h"
#include "base/memory/scoped_vector.h"
#include "chrome/render/web_contents_view_port.h"
#include "chrome/render/render_host/render_widget_host_view_win.h"
#include "chrome/render/render_host/render_widget_host.h"
...
#include "chrome/render/render_host/render_view_host.h"
...
int WebContentsViewWin::test_view = 0;
...
DropData* WebContentsViewWin::GetDropData() const {
  return NULL;//drag_dest_->current_drop_data();
}
...
LRESULT WebContentsViewWin::OnPaint(UINT message, WPARAM wparam, LPARAM lparam, BOOL& handled) {
  CPaintDC dc(hwnd());
  RECT rc = {0};
  ...
  _sntprintf(szText, MAX_PATH, L"test view:  %d ", test_view_id);
  ::DrawText(dc.m_hDC, szText, -1, &rc, DT_LEFT);
  return 0;
}
