...
LU_CSSDocument* g_shared_css_instance_ = NULL;
...
LU_CSSDocument& ResourceBundle::GetShareCssInstance() {
  CHECK(g_shared_css_instance_ != NULL);
  return *g_shared_css_instance_;
}

void ResourceBundle::InitCssSharedInstanceWithPakPath(const string16& path, const string16& file) {
  if(g_shared_css_instance_)
    g_shared_css_instance_->load(path, file);
}

void ResourceBundle::CleanupCssSharedInstance() {
  if (g_shared_css_instance_) {
    delete g_shared_css_instance_;
    g_shared_css_instance_ = NULL;
  }
}

bool ResourceBundle::HasCssSharedInstance() {
  return g_shared_css_instance_ != NULL;
}
...
void ResourceBundle::InitSharedInstance(Delegate* delegate) {
  ...
  g_shared_css_instance_ = new LU_CSSDocument;
...
//   if (!memory.get())
//     return false;
  //base::FilePath file_path(L"D:\\dev\\c-frame\\myapp\\src\\ui\\res\\default_100_percent\\");
  base::FilePath file_path(L"C:\\work\\frame\\myapp\\src\\ui\\res\\default_100_percent\\");
  base::FilePath file_path_res = file_path.Append(GetImageFileNameById(resource_id));

  base::MemoryMappedFile mem_file;
  mem_file.Initialize(file_path_res);
  if(!mem_file.data())
    return false;

  if (DecodePNG(/*memory->front()*/mem_file.data(), mem_file.length()/*memory->size()*/, bitmap, fell_back_to_1x))
    return true;
  ...
  scoped_ptr<SkBitmap> jpeg_bitmap(
      gfx::JPEGCodec::Decode(/*memory->front(), memory->size()*/mem_file.data(), mem_file.length()));
  if (jpeg_bitmap.get()) {
...
bool ResourceBundle::PNGContainsFallbackMarker(const unsigned char* buf,
  ...
  for (;;) {
    if (size - pos < kPngChunkMetadataSize)
      break;
    uint32 length = 0;
    ReadBigEndian(reinterpret_cast<const char*>(buf + pos), &length);
