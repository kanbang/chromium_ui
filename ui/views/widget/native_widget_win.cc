...
void NativeWidgetWin::RestoreFocusOnActivate() {
  ...
  // chrome/ui/views/native_widget_win_interactive_uitest.cc .
  {
...
gfx::NativeRegion NativeWidgetWin::GetWindowNaviteHRGN(const gfx::Size& size) {
  if (GetWidget()->non_client_view())
    return GetWidget()->non_client_view()->GetWindowNaviteHRGN(size);
  return NULL;
}
