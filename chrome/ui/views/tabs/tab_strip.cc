...
#include "chrome/ui/views/tabs/tab_strip_controller.h"
#include "chrome/ui/views/tabs/tab_strip_observer.h"
...
Tab* TabStrip::GetTabAt(Tab* tab, const gfx::Point& tab_in_tab_coordinates) {
  ...
  while (view && view != this && view->id() != 0/*VIEW_ID_TAB*/)
    view = view->parent();

  return view && view->id() == 0/*VIEW_ID_TAB*/ ? static_cast<Tab*>(view) : NULL;
}
