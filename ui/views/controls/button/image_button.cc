...
void ImageButton::SetAttribute(const string16& name, const string16& v) {
  if(name == L"image") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(ImageButton::STATE_NORMAL, icon_image);
    }
    ...
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(ImageButton::STATE_HOVERED, icon_image);
    }
  }
    ...
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(ImageButton::STATE_PRESSED, icon_image);
    }
  }
    ...
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(ImageButton::STATE_DISABLED, icon_image);
    }
  }
  else if(name == L"image-overlay") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetOverlayImage(icon_image);
    }
  }
  else if(name == L"align") {
    if(v == L"left")
      SetImageAlignment(ALIGN_LEFT, v_alignment_);
    else if(v == L"center")
      SetImageAlignment(ALIGN_CENTER, v_alignment_);
    else if(v == L"right")
      SetImageAlignment(ALIGN_RIGHT, v_alignment_);
  }
  else if(name == L"valign") {
    if(v == L"top")
      SetImageAlignment(h_alignment_, ALIGN_TOP);
    else if(v == L"center")
      SetImageAlignment(h_alignment_, ALIGN_MIDDLE);
    else if(v == L"bottom")
      SetImageAlignment(h_alignment_, ALIGN_BOTTOM);
  }
  else
