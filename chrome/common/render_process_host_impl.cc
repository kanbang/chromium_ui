...
#include "chrome/common/child_process_host_impl.h"
...
class SiteProcessMap : public base::SupportsUserData::Data {
 public:
  typedef base::unordered_map<std::string, RenderProcessHost*> SiteToProcessMap;
  SiteProcessMap() {}
...
size_t RenderProcessHost::GetMaxRendererProcessCount() {
  ...
  if (!max_count) {
    ...
    const size_t kMaxRendererProcessCount = 3000;
...
int RenderProcessHostImpl::GetNextRoutingID() {
  static int routingid = 1;
  return routingid++;//widget_helper_->GetNextRoutingID();
}
...
void RenderProcessHostImpl::ReceivedBadMessage() {
  ...
  base::KillProcess(GetHandle(), /*RESULT_CODE_KILLED_BAD_MESSAGE*/0,
                    false);
}
...
StoragePartition* RenderProcessHostImpl::GetStoragePartition() const {
  return NULL;//storage_partition_impl_;
}
...
bool RenderProcessHostImpl::InSameStoragePartition(
  ...
  return false;//storage_partition_impl_ == partition;
}
...
bool RenderProcessHost::ShouldTryToUseExistingProcessHost(
  ...
  return false;/*GetContentClient()->browser()->
