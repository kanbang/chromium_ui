...
void Link::SetAttribute(const string16& name, const string16& v) {
  if(name == L"color-press") {
    SetPressedColor(SkColor(base::GetColor(v)));
  } else if(name == L"underline") {
    SetUnderline(v == L"true");
  } else
    Label::SetAttribute(name, v);
}
