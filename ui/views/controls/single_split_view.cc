...
SingleSplitView::SingleSplitView() :is_horizontal_(true),listener_(NULL),
    divider_offset_(-1),resize_leading_on_bounds_change_(true),resize_disabled_(false) {
#if defined(OS_WIN)
  set_background(
...
void SingleSplitView::SetAttribute(const string16& name, const string16& v) {
  if(name == L"orientation") {
    if(v == L"horizontal")
      set_orientation(HORIZONTAL_SPLIT);
    else
      set_orientation(VERTICAL_SPLIT);
  } else if(name == L"drive-offset") {
    int offset = 0;
    if(base::StringToInt(v, &offset))
      set_divider_offset(offset);
  } else if(name == L"resize-disable")
    set_resize_disabled(v == L"true");
  else
