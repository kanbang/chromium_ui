...
//#include "chrome/ui/app_modal_dialogs/app_modal_dialog.h"
//#include "chrome/ui/app_modal_dialogs/app_modal_dialog_queue.h"
#include "chrome/ui/bookmarks/bookmark_bar_constants.h"
#include "chrome/ui/bookmarks/bookmark_bubble_delegate.h"
#include "chrome/ui/bookmarks/bookmark_bubble_sign_in_delegate.h"
#include "chrome/ui/browser.h"
//#include "chrome/ui/browser_command_controller.h"
//#include "chrome/ui/browser_commands.h"
//#include "chrome/ui/browser_dialogs.h"
#include "chrome/ui/browser_finder.h"
#include "chrome/ui/browser_list.h"
#include "chrome/ui/browser_window_state.h"
//#include "chrome/ui/immersive_fullscreen_configuration.h"
//#include "chrome/ui/ntp_background_util.h"
#include "chrome/ui/omnibox/omnibox_popup_model.h"
#include "chrome/ui/omnibox/omnibox_popup_view.h"
#include "chrome/ui/omnibox/omnibox_view.h"
//#include "chrome/ui/search/search_delegate.h"
//#include "chrome/ui/search/search_model.h"
//#include "chrome/ui/search/search_ui.h"
#include "chrome/ui/tabs/tab_menu_model.h"
#include "chrome/ui/tabs/tab_strip_model.h"
#include "chrome/ui/view_ids.h"
#include "chrome/ui/views/accelerator_table.h"
#include "chrome/ui/views/accessibility/invert_bubble_view.h"
#include "chrome/ui/views/avatar_menu_bubble_view.h"
#include "chrome/ui/views/avatar_menu_button.h"
#include "chrome/ui/views/bookmarks/bookmark_bar_view.h"
#include "chrome/ui/views/bookmarks/bookmark_bubble_view.h"
#include "chrome/ui/views/browser_dialogs.h"
#include "chrome/ui/views/download/download_in_progress_dialog_view.h"
#include "chrome/ui/views/download/download_shelf_view.h"
#include "chrome/ui/views/frame/browser_view_layout.h"
#include "chrome/ui/views/frame/browser_view_layout_delegate.h"
#include "chrome/ui/views/frame/contents_container.h"
#include "chrome/ui/views/frame/immersive_mode_controller.h"
#include "chrome/ui/views/frame/top_container_view.h"
#include "chrome/ui/views/fullscreen_exit_bubble_views.h"
#include "chrome/ui/views/infobars/infobar_container_view.h"
#include "chrome/ui/views/location_bar/location_bar_view.h"
#include "chrome/ui/views/location_bar/location_icon_view.h"
#include "chrome/ui/views/omnibox/omnibox_view_views.h"
#include "chrome/ui/views/omnibox/omnibox_views.h"
#include "chrome/ui/views/password_generation_bubble_view.h"
#include "chrome/ui/views/status_bubble_views.h"
#include "chrome/ui/views/tabs/browser_tab_strip_controller.h"
#include "chrome/ui/views/tabs/tab.h"
#include "chrome/ui/views/tabs/tab_strip.h"
#include "chrome/ui/views/toolbar_view.h"
#include "chrome/ui/views/update_recommended_message_box.h"
#include "chrome/ui/views/website_settings/website_settings_popup_view.h"
#include "chrome/ui/theme/theme_properties.h"
//#include "chrome/ui/window_sizer/window_sizer.h"
...
#include "ash/shell.h"
#include "chrome/ui/ash/ash_util.h"
#endif
...
#include "chrome/ui/views/omnibox/omnibox_view_win.h"
...
//#include "chrome/ui/sync/one_click_signin_bubble_delegate.h"
//#include "chrome/ui/sync/one_click_signin_bubble_links_delegate.h"
#include "chrome/ui/views/sync/one_click_signin_bubble_view.h"
#endif
...
class BookmarkExtensionBackground : public views::Background {
 public:
  ...
  virtual void Paint(gfx::Canvas* canvas, views::View* view) OVERRIDE;
...
bool BrowserView::IsOffTheRecord() const {
  return false;//browser_->profile()->IsOffTheRecord();
}
...
bool BrowserView::IsGuestSession() const {
  return false;//browser_->profile()->IsGuestSession();
}
...
void BrowserView::BookmarkBarStateChanged(
  ...
  if (bookmark_bar_view_.get()) {
    ...
    bool detached_changed = false;/*(new_state == BookmarkBar::DETACHED) ||
...
bool BrowserView::IsDownloadShelfVisible() const {
  return false;//download_shelf_.get() && download_shelf_->IsShowing();
}
...
string16 BrowserView::GetAccessibleWindowTitle() const {
  if (IsOffTheRecord()) {
    return l10n_util::GetStringFUTF16(
        /*IDS_ACCESSIBLE_INCOGNITO_WINDOW_TITLE_FORMAT*/0,
        GetWindowTitle());
  }
...
bool BrowserView::ExecuteWindowsCommand(int command_id) {
...
#if defined(OS_WIN)
  if (command_id == /*IDC_DEBUG_FRAME_TOGGLE*/0)
  ...
  return false;//chrome::ExecuteCommand(browser_.get(), command_id);
}
...
void BrowserView::PaintChildren(gfx::Canvas* canvas) {
  //canvas->FillRect(bounds(), SkColorSetARGB(100, 129, 128, 128));
  //return;
...
//   }
  ...
  contents_web_view_ = new views::WebView(/*browser_->profile()*/NULL);
  ...
  devtools_container_ = new views::WebView(/*browser_->profile()*/NULL);
  devtools_container_->set_id(VIEW_ID_DEV_TOOLS_DOCKED);
  devtools_container_->SetVisible(false);


  //contents_container_ = new ContentsContainer(test_view);
...
//       l10n_util::GetStringUTF16(/*IDS_ACCNAME_WEB_CONTENTS*/0));
...
bool BrowserView::MaybeShowBookmarkBar(WebContents* contents) {
  ...
  if (show_bookmark_bar) {
    if (/*bookmark_bar_view_->IsDetached()*/false)
      new_parent = this;
    else
...
int BrowserView::GetCommandIDForAppCommandID(int app_command_id) const {
  ...
  switch (app_command_id) {
    ...
    // Windows headers.
    case APPCOMMAND_BROWSER_BACKWARD: return /*IDC_BACK*/0;
    case APPCOMMAND_BROWSER_FORWARD:  return /*IDC_FORWARD*/0;
    case APPCOMMAND_BROWSER_REFRESH:  return /*IDC_RELOAD*/0;
    case APPCOMMAND_BROWSER_HOME:     return /*IDC_HOME*/0;
    case APPCOMMAND_BROWSER_STOP:     return /*IDC_STOP*/0;
    case APPCOMMAND_BROWSER_SEARCH:   return /*IDC_FOCUS_SEARCH*/0;
    case APPCOMMAND_HELP:             return /*IDC_HELP_PAGE_VIA_KEYBOARD*/0;
    case APPCOMMAND_NEW:              return /*IDC_NEW_TAB*/0;
    case APPCOMMAND_OPEN:             return /*IDC_OPEN_FILE*/0;
    case APPCOMMAND_CLOSE:            return /*IDC_CLOSE_TAB*/0;
    case APPCOMMAND_SAVE:             return /*IDC_SAVE_PAGE*/0;
    case APPCOMMAND_PRINT:            return /*IDC_PRINT*/0;
    case APPCOMMAND_COPY:             return /*IDC_COPY*/0;
    case APPCOMMAND_CUT:              return /*IDC_CUT*/0;
    case APPCOMMAND_PASTE:            return /*IDC_PASTE*/0;
...
int BrowserView::GetMaxTopInfoBarArrowHeight() {
  ...
  if (!IsFullscreen() &&
    ...
    gfx::Point infobar_top(0, 0/*infobar_container_->GetVerticalOverlap(NULL)*/);
