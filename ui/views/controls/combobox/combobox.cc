...
Combobox::Combobox() : listener_(NULL), invalid_(false), text_border_(new FocusableBorder()),
  disclosure_arrow_(ui::ResourceBundle::GetSharedInstance().GetImageNamed(
  IDR_MENU_DROPARROW).ToImageSkia()),
  dropdown_open_(false) {
    set_focusable(true);
...
ui::SimpleComboBoxModel* Combobox::GetSimpleModel() {
  if(!simple_model_)
    simple_model_.reset(new ui::SimpleComboBoxModel());

  return simple_model_.get();
}
...
Combobox *Combobox::get_named_item(View* parent, const std::string& name)
{
  Combobox *object = NULL;
  if (parent)
    object = static_cast<Combobox*>(parent->GetViewByName(name));
