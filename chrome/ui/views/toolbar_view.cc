...
#include "chrome/ui/views/home_button.h"
#include "chrome/ui/views/location_bar/star_view.h"
#include "chrome/ui/views/outdated_upgrade_bubble_view.h"
#include "chrome/ui/views/wrench_menu.h"
#include "chrome/ui/views/wrench_toolbar_button.h"
...
#if !defined(USE_AURA)
#include "chrome/ui/views/app_menu_button_win.h"
#endif
#endif
...
void ToolbarView::Update(content::WebContents* tab) {
  if (location_bar_)
...
void ToolbarView::GetAccessibleState(ui::AccessibleViewState* state) {
  ...
  state->name = l10n_util::GetStringUTF16(/*IDS_ACCNAME_TOOLBAR*/0);
}
...
content::WebContents* ToolbarView::GetWebContents() {
...
void ToolbarView::Layout() {
  ...
  if (/*show_home_button_.GetValue()*/0) {
  ...
  int browser_actions_width = 0;//browser_actions_->GetPreferredSize().width();
  ...
  app_menu_->SetBounds(location_bar_->x() + location_bar_->width() + browser_actions_width, child_y,
...
bool ToolbarView::ShouldShowUpgradeRecommended() {
  ...
  return false;
#else
  return false;//(UpgradeDetector::GetInstance()->notify_upgrade());
#endif
}
...
void ToolbarView::UpdateAppMenuState() {
  ...
  if (ShouldShowUpgradeRecommended()) {
    accname_app = l10n_util::GetStringFUTF16(
        /*IDS_ACCNAME_APP_UPGRADE_RECOMMENDED*/0, accname_app);
  }
