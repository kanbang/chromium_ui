...
#include "chrome/ui/omnibox/autocomplete_controller.h"
...
#include "base/bind.h"
#include "chrome/ui/omnibox/autocomplete_controller_delegate.h"
...
private:
  ...
  void AddMatchs(string16 url, string16 des, string16 des1);
...
void TestProvider::Run() {
  ...
  //AddResults(0, 3/*kResultsPerProvider*/);
  AddMatchs(L"www.baidu.com", L"http://www.baidu.com", L"百度一下你就知道");
  AddMatchs(L"www.36kr.com", L"http://www.36kr.com", L"IT咨询");
  AddMatchs(L"www.cnbeta.com", L"http://www.cnbeta.com", L"cnbeta-国内咨询网站");
  AddMatchs(L"www.appletuan.com", L"http://www.appletuan.com", L"苹果团");
  AddMatchs(L"www.qq.com", L"http://www.qq.com", L"腾讯网");
  done_ = true;
  DCHECK(listener_);
...
void TestProvider::AddMatchs(string16 info, string16 url, string16 des) {
  //AutocompleteMatch::Type type;
  AutocompleteMatch match(this, 0, false, AutocompleteMatchType::URL_WHAT_YOU_TYPED);

  match.fill_into_edit = info;
  match.destination_url = GURL(url/*UTF16ToUTF8(match.fill_into_edit)*/);
  match.allowed_to_be_default_match = true;

  match.contents = match.fill_into_edit;
  match.contents_class.push_back(
    ACMatchClassification(0, ACMatchClassification::NONE));
  match.description = des;
...
void TestProvider::AddResultsWithSearchTermsArgs(
  ...
  const TemplateURLRef::SearchTermsArgs& search_terms_args) {
    for (int i = start_at; i < num; i++) {
      ...
      match.fill_into_edit = L"http://www.baidu.com/";//prefix_ + UTF8ToUTF16(base::IntToString(i));
      if(i == 1)
        match.destination_url = GURL("http://www.so.com/"/*UTF16ToUTF8(match.fill_into_edit)*/);
      if(i == 2)
        match.destination_url = GURL("http://www.baidu.com/"/*UTF16ToUTF8(match.fill_into_edit)*/);
      if(i == 3)
        match.destination_url = GURL("http://www.so.com/"/*UTF16ToUTF8(match.fill_into_edit)*/);
      match.allowed_to_be_default_match = true;
...
AutocompleteController::AutocompleteController(
    ...
    : delegate_(delegate),
      ...
      in_stop_timer_field_trial_(false),
      done_(true),
      in_start_(false),
      in_zero_suggest_(false),
      profile_(profile) {
  // AND with the disabled providers, if any.
  provider_types =0;//&= ~OmniboxFieldTrial::GetDisabledProviderTypes();
  ...
  TestProvider* tp = new TestProvider(3, L"http://a", profile_, L"t");
  providers_.push_back(tp);
  tp->set_listener(this);
