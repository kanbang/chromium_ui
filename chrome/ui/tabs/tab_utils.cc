...
bool IsPlayingAudio(content::WebContents* contents) {
  ...
  //        .get();
  return false;//audio_indicator && audio_indicator->IsPlayingAudio(contents);
}
...
const gfx::Image& GetTabMediaIndicatorImage(TabMediaState media_state) {
  ...
  switch (media_state) {
    case TAB_MEDIA_STATE_AUDIO_PLAYING:
      return rb.GetNativeImageNamed(/*IDR_TAB_AUDIO_INDICATOR*/0);
    case TAB_MEDIA_STATE_RECORDING:
      return rb.GetNativeImageNamed(/*IDR_TAB_RECORDING_INDICATOR*/0);
    case TAB_MEDIA_STATE_CAPTURING:
      return rb.GetNativeImageNamed(/*IDR_TAB_CAPTURE_INDICATOR*/0);
    case TAB_MEDIA_STATE_NONE:
      break;
  }
  NOTREACHED();
  return rb.GetNativeImageNamed(/*IDR_SAD_FAVICON*/0);
}
