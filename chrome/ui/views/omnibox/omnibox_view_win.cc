...
#include "chrome/ui/omnibox/omnibox_edit_controller.h"
#include "chrome/ui/omnibox/omnibox_edit_model.h"
...
OmniboxViewWin::OmniboxViewWin(OmniboxEditController* controller,
                               ...
                               int font_y_offset)
    : OmniboxView(controller, command_updater),
...
bool OmniboxViewWin::IsItemForCommandIdDynamic(int command_id) const {
  ...
  // search.
  return command_id == /*IDS_PASTE_AND_GO*/6;
}
...
string16 OmniboxViewWin::GetLabelForCommandId(int command_id) const {
  ...
  return l10n_util::GetStringUTF16(
      ...
      /*IDS_PASTE_AND_SEARCH*/0 : /*IDS_PASTE_AND_GO*/0);
}
...
void OmniboxViewWin::ExecuteCommand(int command_id, int event_flags) {
  ...
  if (command_id == /*IDS_PASTE_AND_GO*/2) {
    model()->PasteAndGo(GetClipboardText());
    return;
  }
  if (command_id == /*IDS_SHOW_URL*/3) {
    ShowURL();
    return;
  }
  if (command_id == /*IDC_EDIT_SEARCH_ENGINES*/4) {
...
LRESULT OmniboxViewWin::OnMouseActivate(HWND window,
  ...
  if ((!model()->has_focus() ||
       (model()->focus_state() == 1/*OMNIBOX_FOCUS_INVISIBLE*/)) &&
...
LONG OmniboxViewWin::ClipXCoordToVisibleText(LONG x,
  ...
  // paragraph.
  const bool ltr_text_in_ltr_layout = false;/*(!(pf2.wEffects & PFE_RTLPARA) &&
...
void OmniboxViewWin::BuildContextMenu() {
  ...
  // Set up context menu.
  if (!popup_window_mode_) {
    context_menu_contents_->AddItemWithStringId(/*IDS_UNDO*/0, /*IDS_UNDO*/0);
    context_menu_contents_->AddSeparator(ui::NORMAL_SEPARATOR);
    context_menu_contents_->AddItemWithStringId(/*IDC_CUT*/0, /*IDS_CUT*/0);
  }
  context_menu_contents_->AddItemWithStringId(/*IDC_COPY*/0, /*IDS_COPY*/0);
  if (!popup_window_mode_) {
    context_menu_contents_->AddItemWithStringId(/*IDC_PASTE*/0, /*IDS_PASTE*/0);
    // GetContextualLabel() will override this next label with the
    // IDS_PASTE_AND_SEARCH label as needed.
    context_menu_contents_->AddItemWithStringId(/*IDS_PASTE_AND_GO*/0,
                                                /*IDS_PASTE_AND_GO*/0);
  }
  context_menu_contents_->AddSeparator(ui::NORMAL_SEPARATOR);
  context_menu_contents_->AddItemWithStringId(/*IDS_SELECT_ALL*/0, /*IDS_SELECT_ALL*/0);
