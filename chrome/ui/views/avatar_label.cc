...
AvatarLabelBorder::AvatarLabelBorder() {
  ...
  const int kImages[] = {0, 0, 0, 0, 0, 0, 0, 0, 0};//IMAGE_GRID(IDR_MANAGED_USER_LABEL);
...
void AvatarLabelBorder::Paint(const views::View& view, gfx::Canvas* canvas) {
  ...
  SkPaint paint;
  int kRadius = 2;
  SkColor background_color = view.GetThemeProvider()->GetColor(0);
...
AvatarLabel::AvatarLabel(BrowserView* browser_view)
    : TextButton(NULL,
                 l10n_util::GetStringUTF16(/*IDS_MANAGED_USER_AVATAR_LABEL*/0)),
