...
//   }
  ...
  virtual void OnCrossSiteResponse(
      ...
      int global_request_id) OVERRIDE;
 ...
 private:
  ...
  struct PendingNavigationParams {
    ...
    explicit PendingNavigationParams(const int& global_request_id);
  ...
  bool ShouldSwapEngineForNavigation(
    const NavigationEntry* curr_entry,
    const NavigationEntryImpl* new_entry) const;
  ...
  typedef base::unordered_map<int32, RenderViewHostImpl*> RenderViewHostMap;
