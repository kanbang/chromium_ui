...
int ComputeFormatFlags(int flags, const string16& text) {
  ...
  bool is_rtl = false;
  bool string_contains_strong_rtl_chars = false;
...
void FadeBitmapRect(SkBaseDevice& bmp_device,
                    const gfx::Rect& rect,
                    bool fade_to_right) {
...
void Canvas::DrawStringRectWithFlags(const base::string16& text,
  ...
  HFONT old_font;
  {
    ...
    old_font = (HFONT)SelectObject(dc, font.GetNativeFont());
...
void Canvas::DrawStringWithShadows(const string16& text,
  ...
  DrawStringRectWithFlags(text, font, color, text_bounds, flags);
}
...
void Canvas::DrawStringWithHalo(const string16& text,
  ...
  Size size(w + 2, h + 2);
  Canvas text_canvas(size, 1.0, false);
  SkPaint bkgnd_paint;
  ...
  gfx::ImageSkia skia_bmp = gfx::ImageSkia::CreateFrom1xBitmap(text_bitmap);
  DrawImageInt(skia_bmp, x - 1, y - 1);
}
...
void Canvas::DrawFadeTruncatingString(
  ...
  SizeStringInt(text, font, &total_string_width, &total_string_height, 0,
  ...
  switch (truncate_mode) {
    ...
    case TruncateFadeHeadAndTail:
      ...
      SizeStringInt(clipped_head_string, font,
                    &clipped_width, &clipped_height, 0, flags);
  ...
  Translate(display_rect.OffsetFromOrigin());
