...
#include "chrome/ui/views/omnibox/omnibox_result_view.h"
...
#include "chrome/ui/views/omnibox/omnibox_result_view_model.h"
...
//         return x;
      const int num_runs = 1;/*bidi_line.CountRuns()*/;
      ...
      for (int run = 0; run < num_runs; ++run) {
        ...
        current_run->is_rtl = false;//!is_url && (run_direction == UBIDI_RTL);
      ...
      // Draw the runs.
      for (Runs::iterator i(runs.begin()); i != runs.end(); ++i) {
        const bool reverse_visible_order = false;//(i->is_rtl != base::i18n::IsRTL());
