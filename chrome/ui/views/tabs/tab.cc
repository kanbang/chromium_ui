...
#include "chrome/ui/tabs/tab_resources.h"
#include "chrome/ui/tabs/tab_utils.h"
#include "chrome/ui/view_ids.h"
#include "chrome/ui/views/tabs/tab_controller.h"
#include "chrome/ui/views/theme_image_mapper.h"
#include "chrome/ui/views/touch_uma/touch_uma.h"
...
static const int kMiniTabRendererAsNormalTabWidth =
    /*browser_defaults::kMiniTabWidth*/100 + 30;
...
int Tab::GetMiniWidth() {
  return 100;//browser_defaults::kMiniTabWidth;
}
...
void Tab::PaintIcon(gfx::Canvas* canvas) {
  ...
  if (data().network_state != TabRendererData::NETWORK_STATE_NONE) {
    ...
    gfx::ImageSkia frames(*tp->GetImageSkiaNamed(
        ...
        /*IDR_THROBBER_WAITING*/0 : IDR_THROBBER));
    ...
    // Paint crash favicon.
    ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();
    gfx::ImageSkia crashed_favicon(*rb.GetImageSkiaNamed(/*IDR_SAD_FAVICON*/0));
...
void Tab::PaintTitle(gfx::Canvas* canvas, SkColor title_color) {
  ...
  if (title.empty()) {
    title = data().loading ?
        l10n_util::GetStringUTF16(/*IDS_TAB_LOADING_TITLE*/0) :
        CoreTabHelper::GetDefaultTitle();
  } else {
    Browser::FormatTitleForDisplay(&title);
  }

  if(title_bounds.width() == 0 || title_bounds.height() == 0)
    return;
