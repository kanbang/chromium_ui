...
int Textfield::GetTextDirection() const {
  return native_wrapper_ ?
      native_wrapper_->GetTextDirection() : /*base::i18n::UNKNOWN_DIRECTION*/0;
}
...
void Textfield::SetAttribute(const string16& name, const string16& v) {
  if(name == L"text")
    SetText(v);
  else if(name == L"text-place")
    set_placeholder_text(v);
  else if(name == L"color-place")
    set_placeholder_text_color(SkColor(base::GetColor(v)));
  else if(name == L"color")
    SetTextColor(SkColor(base::GetColor(v)));
  else if(name == L"readonly")
   SetReadOnly(v == L"true");
  else if(name == L"password") {
    if(v == L"true")
      style_ = STYLE_OBSCURED;
    else
      style_ = STYLE_DEFAULT;
  }
  else if(name == L"background-color")
    SetBackgroundColor(SkColor(base::GetColor(v)));
  else if(name == L"margin") {
    std::vector<string16> margins = split_text(v, L" ");
    if(margins.size() == 2) {
      int left, right;
      if(base::StringToInt(margins[0], &left) && base::StringToInt(margins[0], &right))
        SetHorizontalMargins(left, right);
    }
  }
  else if(name == L"vmargin") {
    std::vector<string16> margins = split_text(v, L" ");
    if(margins.size() == 2) {
      int top, bottom;
      if(base::StringToInt(margins[0], &top) && base::StringToInt(margins[0], &bottom))
