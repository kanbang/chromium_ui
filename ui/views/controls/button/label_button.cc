...
void LabelButton::SetAttribute(const string16& name, const string16& v) {
	if(name == L"text") {
		SetText(v);
	}
    ...
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(Button::STATE_NORMAL, *icon_image);
    }
  } else if(name == L"image-hot") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(Button::STATE_HOVERED, *icon_image);
    }
  }
  else if(name == L"image-push") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(Button::STATE_PRESSED, *icon_image);
    }
  }
  else if(name == L"image-disable") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetImage(Button::STATE_DISABLED, *icon_image);
    }
  }
  else if(name == L"elide") {
    if(v == L"none")
      SetElideBehavior(Label::NO_ELIDE);
    else if(v == L"middle")
      SetElideBehavior(Label::ELIDE_IN_MIDDLE);
    else if(v == L"end")
      SetElideBehavior(Label::ELIDE_AT_END);
    else if(v == L"email")
      SetElideBehavior(Label::ELIDE_AS_EMAIL);
  }
  else if(name == L"style") {
    if(v == L"button")
      SetStyle(Button::STYLE_BUTTON);
    else if(v == L"textbutton")
      SetStyle(Button::STYLE_TEXTBUTTON);
    else if(v == L"system")
      SetStyle(Button::STYLE_NATIVE_TEXTBUTTON);
  }
  else if(name == L"multiline") {
    SetTextMultiLine(v == L"true");
  }
  else if(name == L"color") {
    SetTextColor(Button::STATE_NORMAL, SkColor(base::GetColor(v)));
  }
  else if(name == L"color-hot")
    SetTextColor(Button::STATE_HOVERED, SkColor(base::GetColor(v)));
  else if(name == L"color-highlight")
    SetTextColor(Button::STATE_PRESSED, SkColor(base::GetColor(v)));
  else if(name == L"color-disable")
    SetTextColor(Button::STATE_DISABLED, SkColor(base::GetColor(v)));
  else if(name == L"default")
    SetIsDefault(v == L"true");
  else if(name == L"font") {
    int fontindex  = 0;
