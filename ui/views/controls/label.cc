...
void Label::SetAttribute(const string16& name, const string16& v) {
  if(name == L"text")
    SetText(v);
  else if(name == L"tooltip")
    SetTooltipText(v);
...
void Label::OnPaint(gfx::Canvas* canvas) {
  ...
  if(newui_style_) {
    ...
    DrawTextByCss(action, bounds, canvas, text_);
  } else {
    string16 paint_text;
...
int Label::ComputeDrawStringFlags() const {
  ...
  if (directionality_mode_ == AUTO_DETECT_DIRECTIONALITY) {
    ...
    if (direction == 2/*base::i18n::RIGHT_TO_LEFT*/)
