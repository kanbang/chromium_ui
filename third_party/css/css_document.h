...
class LU_IODevice;
class LU_VirtualFileSystem;
class LU_CSSProperty;
class LU_CSSDocument_Impl;
class LU_VirtualDirectory;

/// \brief Cascading Style Sheet (CSS) document.
/// Document class for cascading style sheets (CSS).
/// !group=Core/CSS! !header=core.h!
class CSS_EXPORT LU_CSSDocument
{
/// \name Construction
/// \{
public:
...
public:
	...
	/** <p>The syntax of element is the same as specifying contextual elements
	    ...
	    first in the list.</p>*/
	std::vector<LU_CSSProperty> select(const std::wstring &element);
  std::map<std::wstring, std::wstring> select_map(const std::wstring &element);

	/// \brief Load CSS document from disk.
	///
	/// \param fullname = full filename
	void load(const std::wstring &path, const std::wstring &filename);

	/// \brief Parses a text string and returns the CSS properties in it
	static std::vector<LU_CSSProperty> parse_style_properties(const std::wstring &style_text);
/// \}

/// \name Implementation
/// \{
private:
	scoped_refptr<LU_CSSDocument_Impl> impl;
/// \}
};
