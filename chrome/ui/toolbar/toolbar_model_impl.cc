...
#include "chrome/ui/toolbar/toolbar_model_delegate.h"
...
string16 ToolbarModelImpl::GetText(bool allow_search_term_replacement) const {
  ...
  GURL url(GetURL());
  if (url.spec().length() > 4096/*content::kMaxURLDisplayChars*/)
  ...
  return base::UTF8ToWide(url.spec());//L"todo:toolbarmodel";
}
