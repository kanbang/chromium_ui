...
#include "chrome/ui/views/tabs/tab_drag_controller.h"
...
#include "chrome/ui/views/tabs/dragged_tab_view.h"
#include "chrome/ui/views/tabs/native_view_photobooth.h"
#include "chrome/ui/views/tabs/stacked_tab_strip_layout.h"
...
namespace {
	...
	public:
		...
		virtual void OnPaintBackground(gfx::Canvas* canvas) OVERRIDE {
			...
			canvas->Save();
			bool rtl_ui = false; //kk base::i18n::IsRTL();
			if (rtl_ui) {
...
bool TabDragController::ShouldDetachIntoNewBrowser() {
	...
	return CommandLine::ForCurrentProcess()->HasSwitch(
		/*switches::kTabBrowserDragging*/""); //kk
#endif
}
...
//kk 			OpenURLParams forward_params = params;
...
//kk content::JavaScriptDialogManager*
...
//kk 	content::NotificationService::current()->Notify(
...
void TabDragController::CompleteDrag() {
	...
	if (attached_tabstrip_) {
		...
		if (dock_info_.type() != DockInfo::NONE) {
			...
			case DockInfo::LEFT_OF_WINDOW:
				//kk content::RecordAction(UserMetricsAction("DockingWindow_Left"));
				break;
			case DockInfo::RIGHT_OF_WINDOW:
				//kk content::RecordAction(UserMetricsAction("DockingWindow_Right"));
				break;
			case DockInfo::BOTTOM_OF_WINDOW:
				//kk content::RecordAction(UserMetricsAction("DockingWindow_Bottom"));
				break;
			case DockInfo::TOP_OF_WINDOW:
				//kk content::RecordAction(UserMetricsAction("DockingWindow_Top"));
				break;
			case DockInfo::MAXIMIZE:
				//kk content::RecordAction(
				//kk 	UserMetricsAction("DockingWindow_Maximize"));
				break;
			case DockInfo::LEFT_HALF:
				//kk content::RecordAction(
				//kk 	UserMetricsAction("DockingWindow_LeftHalf"));
				break;
			case DockInfo::RIGHT_HALF:
				//kk content::RecordAction(
				//kk 	UserMetricsAction("DockingWindow_RightHalf"));
				break;
			case DockInfo::BOTTOM_HALF:
				//kk content::RecordAction(
				//kk 	UserMetricsAction("DockingWindow_BottomHalf"));
				break;
			default:
				NOTREACHED();
				break;
