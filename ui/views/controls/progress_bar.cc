...
void ProgressBar::SetAttribute(const string16& name, const string16& v) {
  if(name == L"tooltip") {
    SetTooltipText(v);
  } else if(name == L"range") {
    std::vector<string16> range = split_text(v, L" ");
    if(range.size() == 2) {
      double rmin  = 0.0;
      double rmax = 0.0;
      if(base::StringToDouble(WideToASCII(range[0]), &rmin) 
        && base::StringToDouble(WideToASCII(range[1]), &rmax)) {
          SetDisplayRange(rmin, rmax);
      }
    }
  } else if(name == L"value") {
    double cur_v = 0.0;
    if(base::StringToDouble(WideToASCII(v), &cur_v))
      SetValue(cur_v);
  }
  else
