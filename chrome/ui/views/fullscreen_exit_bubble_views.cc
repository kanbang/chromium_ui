...
void FullscreenExitBubbleViews::FullscreenExitView::UpdateContent(
  ...
  if (fullscreen_bubble::ShowButtonsForType(bubble_type)) {
    ...
    if (bubble_type == FEB_TYPE_BROWSER_FULLSCREEN_EXIT_INSTRUCTION ||
      ...
      accelerator = l10n_util::GetStringUTF16(/*IDS_APP_ESC_KEY*/0);
    } else {
      link_visible = false;
    }
#if !defined(OS_CHROMEOS)
    if (link_visible) {
      link_->SetText(
          l10n_util::GetStringUTF16(/*IDS_EXIT_FULLSCREEN_MODE*/0) +
          UTF8ToUTF16(" ") +
          l10n_util::GetStringFUTF16(/*IDS_EXIT_FULLSCREEN_MODE_ACCELERATOR*/0,
              accelerator));
    }
#endif
