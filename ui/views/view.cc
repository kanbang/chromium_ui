...
gfx::Size View::GetLayoutSize() {
  if(!layout_info_.miss_visible && !visible())
    return gfx::Size();

  gfx::Size size = GetPreferredSize();
  if(layout_info_.layout_width > 0)
    size.set_width(layout_info_.layout_width);
  else if(layout_info_.layout_width == -1) {
    if(parent_)
      size.set_width(parent_->width());
  }

  if(layout_info_.layout_height > 0)
    size.set_height(layout_info_.layout_height);
  else if(layout_info_.layout_height == -1) {
    if(parent_)
      size.set_height(parent_->height());
  }
  return size;
}
...
View* View::GetViewByName(const std::string& name) {
  return const_cast<View*>(const_cast<const View*>(this)->GetViewByName(name));
}

const View* View::GetViewByName(const std::string& name) const {
  if (name == name_)
    return const_cast<View*>(this);

  for (int i = 0, count = child_count(); i < count; ++i) {
    const View* view = child_at(i)->GetViewByName(name);
    if (view)
      return view;
  }
  return NULL;
...
std::string View::GetAction() {
  std::string action = GetClassName();
  if(!name_.empty()) {
	  action += "#";
    action += name_;
  }
...
string16 View::GetAttribute(const string16& name) {
  if(layout_info_.key_values.find(name) != layout_info_.key_values.end()) {
    return layout_info_.key_values[name];
  }

  return L"";
}

void View::SetAttribute(const string16& name, const string16& v) {
  if(name == LayoutProperty::layout_width) {
    if(v == L"fill_parent") {
      layout_info_.layout_width = -1;
    } else if(v == L"wrap_content") {
      layout_info_.layout_width = 0;
    } else {
      base::StringToInt(v, &layout_info_.layout_width);
    }
  } else if(name == LayoutProperty::layout_height) {
    if(v == L"fill_parent") {
      layout_info_.layout_height = -1;
    } else if(v == L"wrap_content") {
      layout_info_.layout_height = 0;
    } else {
      base::StringToInt(v, &layout_info_.layout_height);
    }
  } else if(name == LayoutProperty::layout_miss_visible){
    layout_info_.miss_visible = (v==L"true");
  } else if(name == LayoutProperty::layout_x) {
    base::StringToInt(v, &layout_info_.layout_x);
  } else if(name == LayoutProperty::layout_y) {
    base::StringToInt(v, &layout_info_.layout_y);
  } else if(name == L"newstyle") {
	  SetNewUIStyle(v==L"true");
  } else if(name == L"background") {
    if(v == L"image") {
      set_background(Background::CreateImageBackground());
    } else {
      std::string color = base::GetRGBAColor(v);
      uint64 c;
      if(base::HexStringToUInt64(color, &c)) {
        set_background(Background::CreateSolidBackground(SkColor(c)));
      }
    }
  } else if(name == L"VerticalGradientBackground") {
	  std::vector<string16> bg_c;
	  bg_c = split_text(v, L" ", true);
	  if (bg_c.size() == 2) {
		  uint64 c1, c2;
		  if(base::HexStringToUInt64(base::GetRGBAColor(bg_c[0]), &c1) && base::HexStringToUInt64(base::GetRGBAColor(bg_c[1]), &c2)) {
			  set_background(Background::CreateVerticalGradientBackground(SkColor(c1), SkColor(c2)));
		  }
	  }
  } else if(name == L"HorizontalGradientBackground") {
	  std::vector<string16> bg_c = split_text(v, L" ");
	  if (bg_c.size() == 2) {
		  uint64 c1, c2;
		  if(base::HexStringToUInt64(base::GetRGBAColor(bg_c[0]), &c1) && base::HexStringToUInt64(base::GetRGBAColor(bg_c[1]), &c2)) {
			  //set_background(Background::CreateHorizontalGradientBackground(c1, c2));
		  }
	  }
  } else if(name == L"focusborder") {
	  if(v == L"true") {
		  //set_focus_border(new views::FocusBorder::CreateDashedFocusBorder());
		  set_focusable(true);
	  }
  } else if(name == L"solidborder") {
	  std::vector<string16> sb_c = split_text(v, L" ");
	  if(sb_c.size() == 2) {
		  int thnikness = 0;
		  uint64 c;
		  if(base::StringToInt(sb_c[0], &thnikness) &&
			  base::HexStringToUInt64(base::GetRGBAColor(sb_c[1]), &c)) {
				  set_border(Border::CreateSolidBorder(thnikness, SkColor(c)));
		  }
	  }
  } else if(name == L"enable") {
	  SetEnabled(v == L"true");
  } else if(name == L"group") {
	  int group = -1;
	  if(base::StringToInt(v, &group))
		  SetGroup(group);
  } else if(name == L"name") {
	  SetName(WideToASCII(v));
  } else if(name == L"focusable") {
    set_focusable(v==L"true");
    ...
    if(padding.size() == 4) {
      ...
      if(base::StringToInt(padding[0], &left) &&
        ...
        base::StringToInt(padding[3], &bottom)) {
          content_insets.Set(top, left, bottom, right);
      }
    }
  }
  else {
    layout_info_.key_values[name] = v;
  }
}

std::string View::GetState() {
  return "";
}
...
void View::DrawImageByCss(std::string& action, const gfx::Rect& rc, gfx::Canvas* canvas) {
  if(action.empty())
    return;
  ...
  image-valign:top center bottom
  image-stretch:none,fill,nine
  image-alpha:
  */
  ...
  if(images.find(L"image-src") != images.end())
    base::StringToInt(images[L"image-src"], &id);
  ...
  int state = 0;
  base::StringToInt(images[L"image-sate"], &state);
  if(state <= 0)
    state = 1;

  int index = 0;
  base::StringToInt(images[L"image-index"], &index);

  int width = image->width() / state;
  ...
  if(stretch == L"nine") {
    canvas->DrawImageByStretchLine(*image, rc.x(), rc.y(), rc.width(), rc.height(), index, state);
  } else if(stretch == L"fill") {
    canvas->DrawImageInt(*image, index*width, 0, width, height, rc.x(), rc.y(), rc.width(), rc.height(), false);
    ...
    canvas->DrawImageInt(*image, index*width, 0, width, height, x, y, width, height, false);
  }  
}

namespace {
  int CSS_ComputeDrawStringFlags(const string16& align, bool multiline, bool char_break) {
    int flags = 0;
  ...
  gfx::Rect CSS_GetTextBounds(const gfx::Rect& display_rc, const string16& text,
    const string16& align, bool multiline, gfx::FontList& fontlist, bool char_break) {

    int w = multiline ?
      display_rc.width() : std::numeric_limits<int>::max();
    int h = fontlist.GetHeight();

    // For single-line strings, ignore the available width and calculate how
    // wide the text wants to be.
    int flags = CSS_ComputeDrawStringFlags(align, multiline, char_break);
    if (!multiline)
  ...
  void CSS_CalculateDrawStringParams(string16* paint_text,
    gfx::Rect* text_bounds,
    int* flags, const gfx::Rect& display_rc, bool multiline, int elide,
    const string16& text, gfx::FontList& fontlist, const string16& align, bool char_break)  {
      ...
      *text_bounds = CSS_GetTextBounds(display_rc, text, align, multiline, fontlist, char_break);
      *flags = CSS_ComputeDrawStringFlags(align, multiline, char_break);
      if (!multiline || (elide == 0))
        *flags |= gfx::Canvas::NO_ELLIPSIS;
  }

}
void View::DrawTextByCss(std::string& action, const gfx::Rect& rc, gfx::Canvas* canvas, const string16& text) {
  if(action.empty())
    return;
  ...
  string16 drawtext = text;
  if(drawtext.empty())
    drawtext = testinfos[L"text-text"];
  ...
  if(testinfos.find(L"text-fontstyle") != testinfos.end()) {
    ...
    font = rb.GetFont(ui::ResourceBundle::FontStyle(style));
  } else {
    ...
    if(!font_name.empty() && !font_size.empty()) {
      ...
      font = gfx::Font(WideToASCII(font_name), fs); 
    }
  }

  string16 halo = testinfos[L"text-halo"];
  bool multiline = (testinfos[L"text-multiline"] == L"true");
  int elide = 0;
  base::StringToInt(testinfos[L"text-elide"], &elide);
  bool char_break = (testinfos[L"text-charbreak"] == L"true");
  ...
  CSS_CalculateDrawStringParams(&paint_text, &text_bounds, &flags,
    rc, multiline, elide, drawtext, font_list, align, char_break);
  ...
  if(!halo.empty()) {
    ...
    canvas->DrawStringRectWithShadows(paint_text, font_list,
      cb,
      multiline ? rc : text_bounds, 0, flags, shadows);
  }
...
// Painting --------------------------------------------------------------------
//kk 初步判断拖放tab标签时不能平滑移动是因为这里被阉割 LayerOwner
