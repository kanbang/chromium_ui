...
void CustomFrameView::Init(Widget* frame) {
  ...
  close_button_->SetAccessibleName(
      l10n_util::GetStringUTF16(/*IDS_APP_ACCNAME_CLOSE*/0));

  // Close button images will be set in LayoutWindowControls().
  AddChildView(close_button_);

  minimize_button_ = InitWindowCaptionButton(/*IDS_APP_ACCNAME_MINIMIZE*/0,
      IDR_MINIMIZE, IDR_MINIMIZE_H, IDR_MINIMIZE_P);

  maximize_button_ = InitWindowCaptionButton(/*IDS_APP_ACCNAME_MAXIMIZE*/0,
      IDR_MAXIMIZE, IDR_MAXIMIZE_H, IDR_MAXIMIZE_P);

  restore_button_ = InitWindowCaptionButton(/*IDS_APP_ACCNAME_RESTORE*/0,
