...
string16 NativeTextfieldWin::GetText() const {
  ...
  // "(Google (by default".
  return str;//base::i18n::StripWrappingBidiControlCharacters(str);
}
...
int NativeTextfieldWin::GetTextDirection() const {
  NOTIMPLEMENTED();
  return /*base::i18n::UNKNOWN_DIRECTION*/0;
}
...
bool NativeTextfieldWin::IsCommandIdEnabled(int command_id) const {
  switch (command_id) {
    case /*IDS_APP_UNDO*/0:       return !textfield_->read_only() && !!CanUndo();
    case /*IDS_APP_CUT*/1:        return !textfield_->read_only() &&
                                    !textfield_->IsObscured() && !!CanCut();
    case /*IDS_APP_COPY*/2:       return !!CanCopy() && !textfield_->IsObscured();
    case /*IDS_APP_PASTE*/3:      return !textfield_->read_only() && !!CanPaste();
    case /*IDS_APP_SELECT_ALL*/4: return !!CanSelectAll();
    default:                 NOTREACHED();
                             return false;
  }
}
...
void NativeTextfieldWin::ExecuteCommand(int command_id, int event_flags) {
  ...
  switch (command_id) {
    case /*IDS_APP_UNDO*/0:       Undo();           break;
    case /*IDS_APP_CUT*/1:        Cut();            break;
    case /*IDS_APP_COPY*/2:       Copy();           break;
    case /*IDS_APP_PASTE*/3:      Paste();          break;
    case /*IDS_APP_SELECT_ALL*/4: SelectAll(false); break;
...
void NativeTextfieldWin::BuildContextMenu() {
  ...
  context_menu_contents_->AddItemWithStringId(/*IDS_APP_UNDO*/0, /*IDS_APP_UNDO*/0);
  context_menu_contents_->AddSeparator(ui::NORMAL_SEPARATOR);
  context_menu_contents_->AddItemWithStringId(/*IDS_APP_CUT*/0, /*IDS_APP_CUT*/0);
  context_menu_contents_->AddItemWithStringId(/*IDS_APP_COPY*/0, /*IDS_APP_COPY*/0);
  context_menu_contents_->AddItemWithStringId(/*IDS_APP_PASTE*/0, /*IDS_APP_PASTE*/0);
  context_menu_contents_->AddSeparator(ui::NORMAL_SEPARATOR);
  context_menu_contents_->AddItemWithStringId(/*IDS_APP_SELECT_ALL*/0,
                                              /*IDS_APP_SELECT_ALL*/0);
}
