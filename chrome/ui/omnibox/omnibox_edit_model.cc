...
#include "chrome/ui/omnibox/omnibox_current_page_delegate_impl.h"
#include "chrome/ui/omnibox/omnibox_edit_controller.h"
#include "chrome/ui/omnibox/omnibox_navigation_observer.h"
...
#include "chrome/ui/omnibox/omnibox_controller.h"
#include "chrome/ui/omnibox/autocomplete_result.h"
#include "chrome/ui/omnibox/omnibox_focus_state.h"
#include "chrome/ui/toolbar/toolbar_model.h"
//#include "chrome/ui/search/instant_controller.h"
//#include "chrome/ui/search/search_tab_helper.h"
//#include "chrome/ui/toolbar/toolbar_model.h"
...
void RecordPercentageMatchHistogram(const string16& old_text,
  ...
  if (search_term_replacement_active) {
    if (transition == /*content::PAGE_TRANSITION_TYPED*/0) {
...
OmniboxEditModel::OmniboxEditModel(OmniboxView* view,
  ...
  omnibox_controller_.reset(new OmniboxController(this, profile_));
  delegate_.reset(new OmniboxCurrentPageDelegateImpl(controller));
}
...
//       KeywordIsSelected(), true, &match, NULL);
  ...
  *url = match.destination_url;
  ...
  if (perm_url.SchemeIs("http") &&
      url->SchemeIs("http") && perm_url.host() == url->host()) {
    *write_url = true;
    string16 http = ASCIIToUTF16("http") +
        ASCIIToUTF16("://");
...
void OmniboxEditModel::StopAutocomplete() {
  ...
  popup_model()->view()->ShowPop(false);
}
...
void OmniboxEditModel::GetInfoForCurrentText(AutocompleteMatch* match,
                                            GURL* alternate_nav_url) const {
  DCHECK(match != NULL);
  match->destination_url = GURL(view_->GetText());//delegate_->GetURL();
  if (controller_->GetToolbarModel()->WouldPerformSearchTermReplacement(
    ...
    match->destination_url = delegate_->GetURL();
...
void OmniboxEditModel::SetFocusState(int state) {
