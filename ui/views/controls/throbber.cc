...
void Throbber::SetAttribute(const string16& name, const string16& v) {
  if(name == L"time") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      frame_time_ = (TimeDelta::FromMilliseconds(icon));
    } 
  }
    ...
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      SetFrames(icon_image);
    }
  }
  else if(name == L"paint-stop") {
    paint_while_stopped_ = (v == L"true");
  }
  else
...
void CheckmarkThrobber::SetAttribute(const string16& name, const string16& v) {
  if(name == L"check") {
    SetChecked(v == L"true");
  }
  else
    Throbber::SetAttribute(name, v);
}
