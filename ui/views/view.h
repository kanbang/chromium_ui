...
class VIEWS_EXPORT View :
 ...
 public:
  ...
  struct VIEWLAYOUTINFO {
    ...
    bool GetPropertyBool(const string16& key, bool& value) {
      std::wstring s_;
      if(GetProperty(key, s_)) {
        value = (s_==L"true");
        return true;
      }
    ...
    bool GetPropertyInt(const string16& key, int& value) {
      std::wstring s_;
      if(GetProperty(key, s_)) {
        return base::StringToInt(s_, &value);
      }

      return false;
    }

    bool GetProperty(const string16& key, std::wstring& value) {
      std::map<string16, string16>::iterator itr = key_values.find(key);
      if(itr != key_values.end()) {
        value = itr->second;
        return true;
      }
    ...
    std::wstring GetPropertyIdAndInt(const std::wstring & strKey, int & nValues)
    {
      nValues = 0;
      std::wstring strReturn = L""; 
      std::wstring strProperty;
      GetProperty(strKey, strProperty);
      size_t nPos = strProperty.find(L"@id/");
      if( nPos == 0 ) {
        size_t nsubpos = strProperty.find(L" ");
        strReturn = strProperty.substr(4, nsubpos - nPos - 4);
        std::wstring strDigat = strProperty.substr(nsubpos + 1, -1);
        base::StringToInt(strDigat, &nValues);
      } else {
        base::StringToInt(strProperty, &nValues);
      }

      return strReturn;
    }

    DWORD GetLayoutStyle()
    {
      std::wstring strGravity;
      GetProperty(LayoutProperty::layout_gravity, strGravity);
  
      DWORD dwStyle = 0;
      if(layout_width == -1) dwStyle |= LAYOUT_WIDTH_FILL;
      else if(layout_width == 0) dwStyle |= LAYOUT_WIDTH_WRAP;

      if( layout_height == -1 ) dwStyle |= LAYOUT_HEIGHT_FILL;
      else if( layout_height  == 0 ) dwStyle |= LAYOUT_HEIGHT_WRAP;

      std::vector<string16> sSplit = split_text(strGravity, L";", true);

      // ≤È’“right
      bool bVer = false;
      bool bHor = false;
      for (size_t i = 0; i < sSplit.size(); i++) {
        if( sSplit[i] == L"left" && !bVer ) {
          dwStyle |= LAYOUT_GRAVITY_LEFT;
          bVer = true;
          continue;
        } else if( sSplit[i] == L"right" && !bVer ) {
          dwStyle |= LAYOUT_GRAVITY_RIGHT;
          bVer = true;
          continue;
        } else if( sSplit[i] == L"center" && !bVer ) {
          dwStyle |= LAYOUT_GRAVITY_CENTER;
          bVer = true;
          continue;
        } 

        if( sSplit[i] == L"top" && !bHor ) {
          dwStyle |= LAYOUT_GRAVITY_TOP;
          bHor = true;
          continue;
        } else if( sSplit[i] == L"bottom" && !bHor ) {
          dwStyle |= LAYOUT_GRAVITY_BOTTOM;
          bHor = true;
          continue;
        } else if( sSplit[i] == L"vcenter" && !bHor ) {
          dwStyle |= LAYOUT_GRAVITY_VCENTER;
          bHor = true;
          continue;
        }
      }

      if( !bVer ) dwStyle |= LAYOUT_GRAVITY_LEFT;
      if( !bHor ) dwStyle |= LAYOUT_GRAVITY_TOP;
      return dwStyle;
    }

    int layout_x;
    int layout_y;
    int layout_width;  //-1:fillparent 0,perfer, >0 
    int layout_height; //
    bool miss_visible;
    std::map<string16, string16> key_values;
  };
  ...
  virtual gfx::Insets GetContentInsets() const { return content_insets; }
  ...
  virtual gfx::Size GetLayoutSize();
  ...
  virtual const View* GetViewByName(const std::string& name) const;
  virtual View* GetViewByName(const std::string& name);
  ...
  virtual string16 GetAttribute(const string16& name);

  void SetMeta(bool meta) { meta_control_ = meta; }
  bool IsMeta() { return meta_control_; }
  void SetNewUIStyle(bool style) { newui_style_ = style; }
  bool GetNewUIStyle() { return newui_style_; }
  virtual std::string GetAction();
  virtual std::string GetState();
  VIEWLAYOUTINFO &GetLayoutInfo() { return layout_info_; }
  ...
  // draw--image&text
  static void DrawImageByCss(std::string& action, const gfx::Rect& rc, gfx::Canvas* canvas);
  static void DrawTextByCss(std::string& action, const gfx::Rect& rc, gfx::Canvas* canvas, const string16& text);
