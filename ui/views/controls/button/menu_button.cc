...
void MenuButton::GetAccessibleState(ui::AccessibleViewState* state) {
  ...
  state->default_action = l10n_util::GetStringUTF16(/*IDS_APP_ACCACTION_PRESS*/0);
  state->state = ui::AccessibilityTypes::STATE_HASPOPUP;
}

void MenuButton::SetAttribute(const string16& name, const string16& v) {
  if(name == L"arrow") {
    int icon = 0;
    if(base::StringToInt(v, &icon)) {
      const gfx::ImageSkia* icon_image = ResourceBundle::GetSharedInstance().GetImageNamed(
        icon).ToImageSkia();
      set_menu_marker(icon_image);
    }
  }
  else
    TextButton::SetAttribute(name, v);
}
