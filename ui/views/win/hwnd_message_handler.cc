...
void HWNDMessageHandler::ResetWindowRegion(bool force) {
  ...
  if (IsMaximized()) {
    ...
    new_region = delegate_->GetWindowNaviteHRGN(gfx::Size(window_rect.Width(), window_rect.Height()));
    if(!new_region) {
...
void HWNDMessageHandler::RedrawLayeredWindowContents() {
  ...
  //gfx::Rect rcTest(0, 0, size.cx, size.cy);
  //layered_window_contents_->FillRect(rcTest, 0x9A00FF00);
