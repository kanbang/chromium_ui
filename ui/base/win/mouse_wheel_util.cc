  ...
  while (window != window_under_wheel) {
    ...
    if (current_process != wheel_window_process) {
      if (IsChild(window, window_under_wheel)) {
        ...
        bool has_modifier = HIBYTE(GetKeyState(VK_RBUTTON)) ||
          HIBYTE(GetKeyState(VK_CONTROL));
        if(!has_modifier && ::GetClassName(window_under_wheel, 
          class_name, _countof(class_name))) {
            if (0 == wcscmp(class_name, L"Internet Explorer_TridentCmboBx") ||
              0 == wcscmp(class_name, L"MacromediaFlashPlayerActiveX")) {
                window_under_wheel = GetParent(window_under_wheel);
                class_name[0] = L'\0';
                ::GetClassName(window_under_wheel, class_name, arraysize(class_name));
            }

            if(!wcscmp(class_name, L"Internet Explorer_Server")) {
              wchar_t wnd_classname[128] = { 0 };
              if (::GetClassName(window, wnd_classname, _countof(wnd_classname))) {
                if (wcscmp(wnd_classname, L"Chrome_RenderWidgetHostHWND") != 0) {
                  base::AutoReset<bool> auto_reset_recursion_break(&recursion_break, true);
                  PostMessage(window_under_wheel, WM_MOUSEWHEEL, w_param, l_param);
                  return true;
                }
              }
            }
