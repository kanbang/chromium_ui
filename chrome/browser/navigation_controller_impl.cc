...
#include "chrome/render/render_host/render_view_host_impl.h" // Temporary
...
bool ShouldKeepOverride(const NavigationEntry* last_entry) {
  return false;//last_entry && last_entry->GetIsOverridingUserAgent();
}
...
//     return;
  ...
  switch (params.load_type) {
    ...
    case LOAD_TYPE_BROWSER_INITIATED_HTTP_POST:
      if (!params.url.SchemeIs("http") &&
          !params.url.SchemeIs("https")) {
        NOTREACHED() << "Http post load must use http(s) scheme.";
        return;
      }
      break;
    case LOAD_TYPE_DATA:
      if (!params.url.SchemeIs("chrome"/*chrome::kDataScheme*/)) {
...
size_t NavigationControllerImpl::max_entry_count() {
  ...
  return 100;//kMaxSessionHistoryEntries;
}
